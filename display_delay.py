...
class PyThreadStimulate(threading.Thread, QObject):
    ...
    def __init__(self, bhv_inds:list=None, obj_serial:SerialCommunicator=None):
        ...
        self.to_continue = True
        self.obj_serial = obj_serial
        self.bhv_inds = bhv_inds  # start from 0
        self.countdown = 30
        self.label_int_queue = np.zeros([90], dtype=int) - 1
        self.tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.tcp_socket.connect(('localhost', 20173))
        self.not_include_inds = np.array([5,7,11,16,17,21,22,25,30]) - 1  # start from 0
    ...
    def run(self):
        ...
        while self.to_continue:
            ...
            bool_exclude_bhv_inds = np.isin(self.label_int_queue, self.not_include_inds)

            if np.sum(bool_in_bhv_inds[-30:]) <= 6:
                continue

            if np.sum(bool_exclude_bhv_inds[-70:]) >= 6:
                continue
...
class PyThreadStimulateExclude(threading.Thread, QObject):
    ...
    def run(self):
        ...
        while self.to_continue:
            ...
            bool_in_bhv_inds = np.isin(self.label_int_queue, self.bhv_inds)
            if np.sum(bool_in_bhv_inds[30:]) >= 10:
